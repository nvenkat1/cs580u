...
/*Function to check substring niddle is present in string haystack or not, and if yes return it in buffer array */
int myStrStr(char haystack[50], char niddle[50], char buffer[50]){

#if DEBUG
        printf("\t*** hay[%s]: Size=%d, nid[%s]:Size=%d, buf[%s]:Size=%d\n", haystack, strlen(haystack), niddle, strlen(niddle), buffer, strlen(buff$
#endif
...
#if DEBUG
                printf("\n");                                   //added to format debugging when 2nd for loop desnt execute, first print of niddle w$
#endif
        }//niddle for
#if DEBUG
        printf("\n\tBuffer = %s\n\n", buffer);
...
/*Function to split FN and LN in two different arrays after reading it from FILE*/
